...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/Student414/shadowsocksr-1.git shadowsocksr
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/Student414/ToyoDAdoubi/raw/master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://github.com/Student414/ToyoDAdoubi/raw/master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
